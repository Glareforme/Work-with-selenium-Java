...
public class BrowserSetup {
    ...
    public BrowserSetup(){
        try {
             prop = new Properties();
            FileInputStream path = new FileInputStream(System.getProperty("user.dir")+"/src/Configuation.properties");
            FileInputStream ip = new FileInputStream("src/main/java/config.properties");
          // FileInputStream path = new FileInputStream("src/Configuation.properties");
            prop.load(path);
        } catch (IOException e) {
            e.printStackTrace();
        }
